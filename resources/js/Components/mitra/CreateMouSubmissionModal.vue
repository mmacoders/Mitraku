...
const validateFile = (file: File): boolean => {
  ...
  if (!validTypes.includes(file.type)) {
    form.setError('document', 'Format file tidak didukung. Harap unggah file PDF atau DOCX.')
    return false
  }
  
  if (file.size > maxSize) {
    form.setError('document', 'Ukuran file terlalu besar. Maksimal 2MB.')
    return false
  }
...
const confirmSubmit = () => {
  showConfirmation.value = false
  form.post(route('mitra.mou.store'), {
    forceFormData: true,
    onSuccess: () => {
      toastMessage.value = 'Pengajuan MoU berhasil dikirim'
    ...
    onError: (errors) => {
       Object.keys(errors).forEach(key => {
        ...
        if (key === 'title' || key === 'document') {
          form.setError(key, errors[key])
        }
      })
      toastMessage.value = 'Terjadi kesalahan validasi'
...
const submitForm = () => {
  ...
  if (!form.document) {
    form.setError('document', 'Dokumen MoU harus diunggah.')
    hasErrors = true
  }
              ...
              <div class="flex items-center justify-between">
                ...
                <div>
                  ...
                  <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Isi form berikut untuk mengajukan Memorandum of Understanding.
                  </p>
                </div>
                </div>
                <button
              ...
              <div v-if="showConfirmation" class="mt-8">
                 <div class="text-center">
                   <div class="mx-auto flex items-center justify-center h-16 w-16 rounded-full mb-4"
                     :class="{
                       'bg-rose-50 dark:bg-rose-900/20': form.errors.title || form.errors.document,
                       'bg-amber-50 dark:bg-amber-900/20': !form.errors.title && !form.errors.document
                     }">
                     <svg v-if="form.errors.title || form.errors.document" class="h-8 w-8 text-rose-500 dark:text-rose-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                   ...
                   <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2">
                     {{ form.errors.title || form.errors.document ? 'Form Tidak Lengkap' : 'Konfirmasi Pengajuan MoU' }}
                   </h3>
                   
                   <div class="mt-2 mb-6">
                     <p v-if="form.errors.title || form.errors.document" class="text-gray-600 dark:text-gray-300 leading-relaxed">
                     ...
                     <p v-else class="text-gray-600 dark:text-gray-300 leading-relaxed">
                       Pastikan semua data pengajuan sudah benar sebelum dikirim.
                     </p>
                   </div>
                   
                   <div v-if="form.errors.title || form.errors.document" class="bg-rose-50 dark:bg-rose-900/20 rounded-xl p-4 mb-6 text-left">
                     ...
                     <ul class="space-y-2 text-sm text-rose-700 dark:text-rose-300">
                       ...
                       <li v-if="form.errors.document" class="flex items-start">
                         <svg class="h-5 w-5 text-rose-500 mr-2 mt-0.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                           <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                         </svg>
                         <span>Dokumen: {{ form.errors.document }}</span>
                       </li>
                     </ul>
                   </div>
                   ...
                   <div v-else class="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-4 mb-6 text-left">
                     ...
                     <ul class="space-y-2 text-sm text-gray-600 dark:text-gray-300">
                       ...
                       <li class="flex items-start">
                         ...
                         <span>Dokumen: {{ form.document ? 'Sudah diunggah' : 'Belum diunggah' }}</span>
                       </li>
                     </ul>
                   </div>
                   ...
                   <div class="flex flex-col sm:flex-row gap-3">
                     <button
                       v-if="!(form.errors.title || form.errors.document)"
                       type="button"
                     ...
                     <button
                       ...
                       :class="{
                         'bg-rose-500 hover:bg-rose-600 text-white': form.errors.title || form.errors.document,
                         'bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 px-4 py-3 text-sm font-medium text-gray-700 dark:text-gray-200 shadow-sm hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-all duration-200 ease-in-out': !(form.errors.title || form.errors.document)
                       }"
                       @click="cancelSubmit"
                     >
                       {{ form.errors.title || form.errors.document ? 'Perbaiki Form' : 'Periksa Kembali' }}
                     </button>
                   </div>
                 </div>
              </div>
              ...
              <form v-else @submit.prevent="submitForm" class="mt-8 space-y-6">
                <div>
                  ...
                  <div class="mt-1">
                    <input
                      ...
                      placeholder="Masukkan judul MoU Anda..."
                ...
                <div>
                  ...
                  <div 
                    ...
                    :class="{
                      'bg-indigo-50/50 dark:bg-indigo-900/20 border-indigo-300 dark:border-indigo-700': isDragging,
                      'hover:bg-indigo-50/50 dark:hover:bg-indigo-900/20': !isDragging
                    }"
                  ...
                  >
                    ...
                    <div class="text-center">
                      ...
                      <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                        Klik atau seret file PDF/DOCX
                      </p>
                    </div>
                  </div>

                  <div v-if="form.document" class="mt-2 flex items-center justify-between p-2 bg-gray-50 rounded-lg dark:bg-gray-700">
                    <div class="flex items-center">
                      <span class="text-lg mr-2">
                        {{ getFileIcon(form.document.type) }}
                      </span>
                      <div class="text-xs">
                  ...
                  <p v-if="form.errors.document" class="text-sm text-rose-400">
