            ...
            <DialogPanel class="relative transform overflow-hidden rounded-2xl bg-white dark:bg-gray-800 shadow-xl transition-all sm:w-full sm:max-w-md">
              <div class="px-6 py-6 sm:px-8 sm:py-8">
                ...
                <div class="space-y-6">
                  ...
                  <!-- Rejection Reason (only shown when status is ditolak) -->
                  <div v-if="formData.status === 'ditolak'" class="mt-4">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                      Alasan Penolakan
                    </label>
                    <textarea
...
const isFormValid = computed(() => {
  ...
  if (formData.value.status === 'ditolak') {
    return !!formData.value.rejectionReason
  }
...
const updateStatus = () => {
  ...
  router.post(route('admin.mou.update-status', props.mou.id), data, {
    onSuccess: (response) => {
      isProcessing.value = false
      emit('success')
      closeModal()
      // Show notification
      alert('Status MoU berhasil diperbarui!')
    },
    onError: (errors) => {
      isProcessing.value = false
      // Handle error
